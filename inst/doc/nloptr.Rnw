...
To install the R interface to NLopt, you need to be able to compile R packages from source. On Linux a compiler is usually available by default, on Windows platforms Rtools is needed to compile R packages from source\footnote{\texttt{http://www.murdoch-sutherland.com/Rtools/}}. I assume you downloaded the file \texttt{nloptr.zip} and extracted the contents somewhere, for instance in \texttt{c:/Rpackages/nloptr}.

\subsection{Linux}
For installation on Linux 
\begin{enumerate}
\item Follow the notes on 
\begin{verbatim}
http://ab-initio.mit.edu/wiki/index.php/NLopt_Installation
\end{verbatim}
to install NLopt. 
\item Then, depending on the settings you used to install NLopt, you may have to change the lines containing \texttt{NLOPT\_LIBS} and \texttt{NLOPT\_INCL} in \texttt{nloptr/src/Makevars}. 
...
\subsection{Windows}
For installation on Windows we use the precompiled binary available at
\begin{verbatim}
