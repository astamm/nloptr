...
is.nloptr <- function(x) {
  ...
  if (is.null(x)) {
    return(FALSE)
  } # Proper use of return to break out early :)
  if (!is.list(x)) {
    return(FALSE)
  }
  ...
  if (!is.function(x$eval_f)) {
    stop("eval_f is not a function")
  }
  ...
  if (lx0 != length(x$lower_bounds)) {
    stop("length(lb) != length(x0)")
  }
  if (lx0 != length(x$upper_bounds)) {
    stop("length(ub) != length(x0)")
  }
  ...
  f0 <- x$eval_f(x$x0)
  if (is.list(f0)) {
    if (is.na(f0$objective)) {
      stop("objective in x0 returns NA")
    }
    if (anyNA(f0$gradient)) {
      stop("gradient of objective in x0 returns NA")
    }
