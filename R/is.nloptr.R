...
is.nloptr <- function(x) {
    ...
    list_algorithms <- c("NLOPT_GN_DIRECT", "NLOPT_GN_DIRECT_L",
                         "NLOPT_GN_DIRECT_L_RAND", "NLOPT_GN_DIRECT_NOSCAL",
                         "NLOPT_GN_DIRECT_L_NOSCAL",
                         "NLOPT_GN_DIRECT_L_RAND_NOSCAL", "NLOPT_GN_ORIG_DIRECT",
                         "NLOPT_GN_ORIG_DIRECT_L", "NLOPT_GD_STOGO",
                         "NLOPT_GD_STOGO_RAND", "NLOPT_LD_SLSQP",
                         "NLOPT_LD_LBFGS_NOCEDAL", "NLOPT_LD_LBFGS",
                         ...
                         "NLOPT_GD_MLSL_LDS", "NLOPT_LD_MMA", "NLOPT_LD_CCSAQ",
                         ...
                         "NLOPT_LN_BOBYQA", "NLOPT_GN_ISRES", "NLOPT_GN_ESCH")
    ...
    if (is.list(f0)) {
        ...
        if (x$options$algorithm %in% list_algorithmsN) {
            warning("a gradient was supplied for the objective function, ",
                    "but algorithm ", x$options$algorithm,
    ...
    if (x$options$algorithm %in%
        ...
        if (is.null(x$local_options)) {
            stop("The algorithm ", x$options$algorithm, " needs a local ",
                 "optimizer; specify an algorithm and termination condition ",
                 "in local_opts")
        }
    }
