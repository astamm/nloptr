...
#        indices : calculate finite difference approximation only for .x-indices in this vector (optional)
...
finite.diff <- function(func, .x, indices = seq_along(.x),
    ...
    vapply(indices, approx.gradf.index, double(length(fx)), .x = .x,
           func = func, fx = fx, stepSizeVec = stepSizeVec, ...)
}
